...
class OneD_Unet(nn.Module):
    def __init__(
            ...
            *,
            ...
            # TODO add cross-attn, doesnt work yet...whether or not to have final conv layer
        ...
        if self.beginning_and_final_conv_present:
            ...
            self.init_conv = CrossEmbedLayer(init_channels, dim_out=init_dim,
                                             ...
                                             stride=1) if init_cross_embed else nn.Conv1d(init_channels, init_dim,
        ...
        for ind, ((dim_in, dim_out), layer_num_resnet_blocks, groups, layer_attn, layer_attn_depth, layer_cross_attn,
            ...
            if not memory_efficient:
                post_downsample = downsample_klass(current_dim, dim_out) if not is_last else Parallel(
                    nn.Conv1d(dim_in, dim_out, 3, padding=1), nn.Conv1d(dim_in, dim_out, 1))
        ...
        self.mid_attn = EinopsToAndFrom('b c h', 'b h c',
        ...
        if self.beginning_and_final_conv_present:
            ...
            self.final_conv = nn.Conv1d(final_conv_dim_in, self.channels_out, final_conv_kernel_size,
    ...
    def forward(
        ...
        if exists(text_embeds) and self.cond_on_text:
            ...
            # this many down sample the token dim
