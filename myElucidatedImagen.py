...
class ElucidatedImagen(nn.Module):
    def __init__(
        ...
        # sets the seeds to load from
        self.seeds  = np.load("data/seeds.npy")
        self.TestSeedInd = 0
        self.TrainSeedInd = 0
    ...
    def preconditioned_network_forward(
        ...
        # data is passed to the unet
        # first is some scalling of the noisey images
        # next is the "time"
    ...
    def noise_distribution(self, P_mean, P_std, batch_size,verbose=False):

        if self.IsTrain:
            # get the seed for this batch
            if self.TrainSeedInd>=len(self.seeds ):
                self.TrainSeedInd=0
            self.seed  = self.seeds [self.TrainSeedInd]
            self.TrainSeedInd += 1
        else:
            if self.TestSeedInd>=len(self.seeds ):
                self.TestSeedInd=0
            self.seed  = self.seeds [self.TestSeedInd]
            self.TestSeedInd += 1
            # print("current val seed " + str(self.seed))
    ...
    def forward(
        ...
        # get prediction # unet_kwargs holds the conditioing
