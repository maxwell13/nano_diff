...
class ProteinDesigner_A(nn.Module):
    def __init__(self, timesteps=10, dim=32, pred_dim=25, loss_type=0, elucidated=False,
        ...
        # brightness bins
        self.boundaries =  torch.tensor( np.loadtxt("data/log_bins.csv") ).to(self.device)
    ...
    def forward(self, cond, output, unet_number=1):  # sequences=conditioning, output=prediction
        ...
        # bins the one-hot encoding
        brightBins = F.one_hot(torch.bucketize(  logB ,self.boundaries), num_classes=len(self.boundaries)+2)

        cond = torch.cat((cond[:,:-1],    brightBins),1)

        # creates an embedding for each position in the input
        cond = cond.unsqueeze(2)
        cond = self.fc_embed2(cond)
        # for debugging
        pos_matrix_i_ = self.pos_matrix_i
        pos_matrix_i_ = pos_matrix_i_.repeat(cond.shape[0], 1)

        pos_matrix_i_ = pos_matrix_i_.to(self.device)
